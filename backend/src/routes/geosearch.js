...
router.get('/', async (req, res) => {
    try {
        ...
        const mapData = soilData.rows.map(sample => {
            ...
            if (sample.lat_lon) {
                ...
                if (typeof sample.lat_lon === 'string') {
                    ...
                    // Format: {x: lon, y: lat}
                    latitude = sample.lat_lon.y;
                    longitude = sample.lat_lon.x;
                } else {
        ...
        logger.info(`Geosearch: Retrieved ${validSamples.length} valid soil samples out of ${mapData.length} total samples`);
...
router.get('/:id', async (req, res) => {
    try {
        ...
        logger.info(`Geosearch: Retrieved detailed data for soil sample ${soilId}`);
        ...
        logger.error(`Error retrieving soil sample ${req.params.id}:`, error);
        res.status(500).json({
            success: false,
