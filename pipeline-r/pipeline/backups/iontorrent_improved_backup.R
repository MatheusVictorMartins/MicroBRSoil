...
names(dadaFs) <- sample.names
  ...
  # --------------------------------------------------
  # Step 5: Taxonomy assignment (multithread + faster rank summary)
  # --------------------------------------------------
cat("Step 5: Taxonomy assignment\n")
valid_ref <- !is.null(path2) && file.exists(path2)

# configure threads for DADA2 taxonomy (optional but recommended)
if (isTRUE(use_multithread) && requireNamespace("BiocParallel", quietly = TRUE)) {
  n_cores_tax <- min(8, max(1, parallel::detectCores() - 2))  # adjust cap (8) if you want
  BiocParallel::register(BiocParallel::MulticoreParam(workers = n_cores_tax))
  cat("  Taxonomy threads:", n_cores_tax, "\n")
}

taxa <- tryCatch({
  if (valid_ref) {
    assignTaxonomy(seqtab.nochim, path2, multithread = isTRUE(use_multithread))
  } else {
...
# Rank coverage summary (faster base R; fallback to Genus)
