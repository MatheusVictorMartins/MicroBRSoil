...
cat("Exists:", file.exists(Sys.getenv("VSEARCH_BIN")), "\n")
cat("Executable:", file.access(Sys.getenv("VSEARCH_BIN"), 1) == 0, "\n")
...
# threads
n_cores_tax <- if (isTRUE(use_multithread))
  min(8, max(1, parallel::detectCores() - 2)) else 1
...
parse_tax <- function(x) {
  ...
  out <- matrix(NA_character_, nrow = length(x), ncol = 7,
                dimnames = list(NULL, ranks))
  x[is.na(x)] <- NA_character_
  spl <- strsplit(x, ";", fixed = TRUE)
  for (i in seq_along(spl)) {
    s <- trimws(spl[[i]])
    if (length(s)) out[i, seq_len(min(7, length(s)))] <- s[seq_len(min(7, length(s)))]
  }
  out
}

build_ref <- function(train_fa, ref_fa, tax_tsv) {
  in_con <- file(train_fa, "r")
  fa_con <- file(ref_fa, "w")
  tx_con <- file(tax_tsv, "w")
  on.exit({
    close(in_con); close(fa_con); close(tx_con)
  }, add = TRUE)

  writeLines("FeatureID\tTaxon", tx_con)
  i <- 0L
  repeat {
    l <- readLines(in_con, n = 1)
    if (!length(l)) break
    if (startsWith(l, ">")) {
      i <- i + 1L
      id <- paste0("REF_", i)
...
taxa <- tryCatch({
  ...
  # build ref DB (taxonomy-only headers)
  n_ref <- build_ref(path2, ref_fa, ref_tax)
  cat("  Reference sequences:", n_ref, "\n")

  # ASV sequences
  asv_seqs <- colnames(seqtab.nochim)
  stopifnot(length(asv_seqs) > 0)

  cat(sprintf(
    "  ASV length: min=%d median=%d max=%d\n",
  ...
  if (!file.exists(blast6) || file.info(blast6)$size == 0) {
    stop("vsearch produced no hits")
  }

  # read hits
  hits <- read.table(blast6, sep = "\t", stringsAsFactors = FALSE)
  hits <- hits[!duplicated(hits[, 1]), , drop = FALSE]
  colnames(hits)[1:2] <- c("qid", "sid")
  cat(sprintf("  vsearch hits: %d / %d ASVs\n", nrow(hits), length(asv_seqs)))

  # map to taxonomy
  ref_tab <- read.table(ref_tax, sep = "\t", header = TRUE, stringsAsFactors = FALSE)
  tax_map <- setNames(ref_tab$Taxon, ref_tab$FeatureID)

  sid <- setNames(hits$sid, hits$qid)
  tax_str <- tax_map[sid[q_ids]]

  taxa_mat <- matrix(
    NA_character_,
    nrow = length(asv_seqs),
    ncol = 7,
    dimnames = list(asv_seqs,
                    c("kingdom","phylum","class","order","family","genus","species"))
  )
  taxa_mat[,] <- parse_tax(tax_str)
...
# --------------------------------------------------
# Rank coverage summary (unchanged)
# --------------------------------------------------
...
if (!is.null(numer_of_taxa_assigned_by_rank) && length(numer_of_taxa_assigned_by_rank)) {
  i <- tail(which(numer_of_taxa_assigned_by_rank >= 2), 1)
  if (!length(i)) i <- which.max(numer_of_taxa_assigned_by_rank)
  rank_name <- names(numer_of_taxa_assigned_by_rank)[i]
}
